"""
exporter.py — CookD Export Dialog (formerly fadhil/exporter.py)
"""
...
def _save_table(table_name, output_path):
    db = TinyDB(DB_PATH)
...
def _open_save_dialog(filename, callback):
    def run():
        try:
            ...
            path = filedialog.asksaveasfilename(title="Simpan Export", defaultextension=".json", filetypes=[("JSON file", "*.json"), ("Semua file", "*.*")], initialfile=filename)
            root.destroy()
            if path:
                callback(path)
        except Exception as ex:
            print(f"[exporter] error: {ex}")
...
def export_my_recipes(page, on_done=None):
...
def show_export_dialog(page, on_done=None):
    ...
    for table_key, label in TOKO_TABLES.items():
        cb = ft.Checkbox(label=label, value=False, fill_color=ORANGE, check_color="#FFFFFF", on_change=lambda e, k=table_key: toko_checks[k].update({"val": e.control.value}))
        toko_rows.append(cb)

    advanced_section = ft.Container(
        content=ft.Column(
            controls=[ft.Container(height=4), ft.Divider(color=tBG4(), height=1), ft.Container(height=4), ft.Text("Pilih data toko yang mau diekspor:", color=tTEXT2(), size=12)] + toko_rows,
            spacing=6, tight=True,
        ),
        visible=False, animate_size=ft.Animation(200, ft.AnimationCurve.EASE_IN_OUT),
    )

    advanced_btn = ft.TextButton("Export data toko lainnya", style=ft.ButtonStyle(color=tTEXT3()))

    def toggle_advanced(e):
        show_advanced["val"] = not show_advanced["val"]
        advanced_section.visible = show_advanced["val"]
        advanced_btn.text = "Sembunyikan" if show_advanced["val"] else "Export data toko lainnya"
    ...
    dialog = ft.AlertDialog(
        ...
        title=ft.Row(
            controls=[
                ft.Container(content=ft.Icon(ft.Icons.DOWNLOAD_OUTLINED, color=ORANGE, size=22), bgcolor=tBG4(), border_radius=ft.BorderRadius.all(8), padding=ft.Padding.all(6)),
                ft.Column(controls=[ft.Text("Export", color=tTEXT(), weight=ft.FontWeight.BOLD, size=16), ft.Text("Simpan data ke file JSON", color=tTEXT3(), size=11)], spacing=1),
            ],
                    ...
                    ft.Container(
                        content=ft.Row(
                            controls=[
                                ...
                                ft.Column(controls=[ft.Text("My Recipes", color=tTEXT(), weight=ft.FontWeight.BOLD, size=14), ft.Text("Resep yang kamu simpan", color=tTEXT2(), size=11)], spacing=2, expand=True),
                            ],
                            spacing=12, vertical_alignment=ft.CrossAxisAlignment.CENTER,
                        ),
                        bgcolor=f"{ORANGE}22", border=ft.Border.all(1, ORANGE), border_radius=ft.BorderRadius.all(10), padding=ft.Padding.all(12),
                    ),
                    advanced_btn, advanced_section,
                ],
        ...
        actions=[
            ...
            ft.ElevatedButton("Export", bgcolor=ORANGE, color="#FFFFFF", style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8)), on_click=do_export),
        ],
