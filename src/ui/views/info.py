"""
info.py — Halaman Info CookD
(formerly zaky/info.py)

Halaman Info: tentang aplikasi, cara menggunakan, dan sumber data.
"""

import flet as ft
from src.core.theme import theme_mgr, ORANGE, GREEN, BLUE, AMBER, WHITE
from src.core.theme import TOK_COLOR, ALFA_COLOR, AEON_COLOR
...
def InfoPage(page: ft.Page) -> ft.Container:
    ...
    def _build_scroll_content() -> ft.Column:
        """Build isi halaman dengan warna tema saat ini. Aman dipanggil berulang."""
        ...
        hero = ft.Container(
            content=ft.Row(
                controls=[
                        ...
                        content=ft.Stack(
                            controls=[
                                ...
                                ft.Container(
                                    ...
                                    content=ft.Icon(ft.Icons.SEARCH_ROUNDED, color=WHITE, size=32),
                                ),
                            ],
                        ),
                    ...
                    ft.Column(
                        controls=[
                            ft.Text("Tentang CookD", size=24, weight=ft.FontWeight.BOLD,
                            ...
                            ft.Text(
                                ...
                                size=13, color="#FFDDCC", font_family="Font", max_lines=3,
                            ),
                        ],
            ...
            shadow=ft.BoxShadow(spread_radius=0, blur_radius=20,
                                color="#66E8440A", offset=ft.Offset(0, 6)),
        )
        ...
        def step_item(number: int, title: str, description: str) -> ft.Container:
            return ft.Container(
                content=ft.Row(
                    controls=[
                        ft.Container(
                            content=ft.Text(str(number), color=WHITE, size=14,
                                            weight=ft.FontWeight.BOLD, font_family="Font",
                                            text_align=ft.TextAlign.CENTER),
                            width=34, height=34, bgcolor=ORANGE,
                        ...
                        ft.Column(
                            controls=[
                                ...
                                ft.Text(description, size=12, color=TEXT2(),
                                        font_family="Font", max_lines=3),
                            ],
        ...
        cara_menggunakan = ft.Container(
            content=ft.Column(
                controls=[
                    ft.Container(
                        content=ft.Text("Cara Menggunakan", size=16,
                                        weight=ft.FontWeight.BOLD, color=TEXT(),
                                        font_family="Font"),
                        padding=ft.Padding.symmetric(horizontal=20, vertical=16),
                        border=ft.Border.only(bottom=ft.BorderSide(1, BORDER())),
                    ),
                    step_item(1, "Masukkan Bahan",
                              "Buka Recipe Finder, ketik bahan yang kamu punya, pisahkan "
                              "dengan koma. Contoh: \"bawang putih, tomat, garam, telur\""),
                    step_item(2, "Pilih Resep",
                              "Resep ditampilkan dengan persentase kecocokan bahan. "
                              "Hijau = semua bahan ada, kuning = hampir lengkap."),
                    step_item(3, "Kalkulasi Harga",
                              "Di halaman detail resep, klik \"Kalkulasi Harga Bahan\" untuk "
                              "melihat perbandingan harga dari 3 toko. Klik tiap bahan untuk "
                              "popup harga detail dan tombol beli langsung."),
                    step_item(4, "Simpan Resep",
                              "Klik ♥ untuk menyimpan resep ke My Recipes agar mudah diakses kembali."),
                ],
                spacing=0,
            ),
            bgcolor=BG2(), border_radius=ft.BorderRadius.all(14),
            ...
            return ft.Container(
                content=ft.Row(
                    controls=[
                        ...
                        ft.Text(label, size=12, color=TEXT(), font_family="Font",
        ...
        sumber_data = ft.Container(
            content=ft.Column(
                controls=[
                    ft.Container(
                        content=ft.Text("Sumber Data", size=16, weight=ft.FontWeight.BOLD,
                                        color=TEXT(), font_family="Font"),
                        padding=ft.Padding.symmetric(horizontal=20, vertical=16),
                        border=ft.Border.only(bottom=ft.BorderSide(1, BORDER())),
                    ),
                        ...
                        content=ft.Column(
                            controls=[
                                ...
                                ft.Text(
                                    ...
                                    size=11, color=TEXT3(), font_family="Font", italic=True,
                                ),
                            ],
                            spacing=14,
                        ),
        ...
        footer = ft.Container(
            content=ft.Text("CookD v0.0.1", size=12, color=TEXT3(),
        ...
        return ft.Column(
            controls=[hero, cara_menggunakan, sumber_data, footer],
    ...
    outer = ft.Container(
        ...
        padding=ft.Padding.symmetric(horizontal=28, vertical=24),
        content=_build_scroll_content(),
    )

    def _on_theme_change():
        outer.bgcolor = BG()
        outer.content = _build_scroll_content()
        try: outer.update()
        except Exception: pass

    # Daftarkan listener SEKALI — cek sentinel di atribut fungsi
    if not any(getattr(f, "_info_listener", False) for f in theme_mgr._listeners):
        _on_theme_change._info_listener = True
